...
6. 工具路径只认 tool availability；缺则 bootstrap（仅 manifest 能力）
...
python3 novahaku/case-review/scripts/review_case.py work/<case> --verify-hashes --strict
```
...
```bash
bash novahaku/scripts/master-route.sh --hint "<用户任务>"
bash novahaku/scripts/master-route.sh --hint "<用户任务>" --project-root "/path/to/analysis-project"
bash novahaku/scripts/case-init.sh --hint "<用户任务>" --case-name "my-case"
bash novahaku/scripts/case-init.sh --hint "<用户任务>" --case-name "my-case" --project-root "/path/to/analysis-project"
# 本地离线样本：
bash novahaku/scripts/case-init.sh --hint "offline apk" --case-name "my-sample" --preset offline-sample --sample ./app.apk
# ACT 前轻量 scope 门禁（--force 为兼容参数，不能绕过硬门）：
bash novahaku/scripts/case-guard.sh --case-root work/my-sample
# 路由 parity：
bash novahaku/scripts/test-routing.sh
bash novahaku/scripts/test-bootstrap-manifest.sh
python3 novahaku/case-review/scripts/review_case.py work/<case> --verify-hashes --strict
```
...
RULES.md → MASTER-ROUTING.md → PRIMARY SKILL.md
  ...
  → tool availability reference → bootstrap → ACT
```
