...
2. `NOW`：读取 `../tool availability reference`，校验工具可用性和实际路径。
3. `NEXT`：缺工具时调用 bootstrap，不要猜路径。
4. `ACT`：进入“工作流”第一步并执行，不要停在确认状态。

## 任务完成自检（声称完成前 MUST 通过）
- □ 我是否执行了工作流中的每一步（而不是只阅读）？
- □ 我是否基于 `tool availability` 使用了真实工具路径？
...
3. `NEXT`: 读取 `../tool availability reference`，校验工具可用性和实际路径
...
### 4.3 在 `tool discovery script` 中注册脚本引用

打开 `novahaku/scripts/tool discovery script`，在 `$scriptRefs` 哈希表中添加：
...
1. 在 `novahaku/tests/routing-benchmark.json` **先**加一条（最好中英各一）失败用例
2. 只改 `novahaku/config/routing.json`（`routes` + `priority`）
3. 同步 `novahaku/MASTER-ROUTING.md` 优先级表（顺序必须与 `priority` 一致）
...
打开 `novahaku/SKILL.md` 模块表；跑 `extract-summaries.ps1` 重生 `INDEX.md`。
...
**Windows**：
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_ROOT>\skills\scripts\tool discovery script"
```

**Kali Linux**：
```bash
bash "<项目根目录>/kali/scripts/tool discovery script"
```

确认新工具出现在 `tool availability reference` 和 `tool manifest` 中。
...
- [ ] `novahaku/scripts/tool discovery script` 的 `$scriptRefs` 已更新
...
- [ ] 运行 refresh-tool availability 后新工具出现在索引中
...
- [ ] 运行 `tool discovery script` 后，capability 视图中能看到新 MCP 的注册和在线状态
