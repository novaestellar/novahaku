...
# 用法：
#   powershell -NoProfile -ExecutionPolicy Bypass -File scripts/reverse-skill/extract-summaries.ps1
#   powershell -File scripts/reverse-skill/extract-summaries.ps1 -Check   # 只校验不写（CI 模式，不一致 exit 1）
param(
    [switch] $Check
)
...
[void]$sb.AppendLine('')
[void]$sb.AppendLine('> 本文件由 `scripts/reverse-skill/extract-summaries.ps1` 自动生成，**请勿手改**。')
...
[void]$sb.AppendLine('```')
foreach ($r in $rows) {
    [void]$sb.AppendLine(("novahaku/{0}/" -f ($r.Path -replace '\\', '/')))
}
[void]$sb.AppendLine('```')
[void]$sb.AppendLine('')
[void]$sb.AppendLine('## 路由')
[void]$sb.AppendLine('')
[void]$sb.AppendLine('PRIMARY 路由由 `config/routing.json`（唯一事实源）驱动，用 `master-route.ps1 -Hint "<任务>"` 分诊。')
