...
[void]$sb.AppendLine(("- primary_skill: novahaku/{0}" -f $primaryPath))
...
foreach ($d in $uniq) {
    if ($d -ne $primary) { [void]$sec.Add(("novahaku/{0}" -f $cfg.routes.$d.skill)) }
}
...
[void]$sb.AppendLine('')
[void]$sb.AppendLine('1. docs/MASTER-ROUTING.md')
[void]$sb.AppendLine(("2. novahaku/{0}" -f $primaryPath))
[void]$sb.AppendLine('')
...
Write-Host ("PRIMARY -> novahaku/{0}" -f $primaryPath) -ForegroundColor Green
