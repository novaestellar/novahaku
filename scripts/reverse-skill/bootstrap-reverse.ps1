...
# Register a directory in the *persistent* per-user PATH, not just this process.
#
# The install paths below used to do `$env:PATH = "$dir;$env:PATH"`, which only
# affects the current PowerShell process. Bootstrap then reported
# `status: ready`, and the tool was gone the moment the shell exited. Tools this
# script installs (jadx, apktool) never reached the machine PATH; tools installed
# by hand (radare2, yara) did. Writing to the User scope makes "ready" mean what
# it says. Idempotent: an entry already present is left alone.
function Add-ReversePersistentUserPath {
    param([Parameter(Mandatory = $true)][string]$Directory)

    if ([string]::IsNullOrWhiteSpace($Directory)) { return }
    if (-not (Test-Path -LiteralPath $Directory)) { return }

    $sepChars = [char[]]@([char]92, [char]47)
    $normalized = $Directory.TrimEnd($sepChars)

    $processEntries = @($env:PATH -split ';' | Where-Object { $_ -ne '' })
    if (-not ($processEntries | Where-Object { $_.TrimEnd($sepChars) -ieq $normalized })) {
        $env:PATH = "$normalized;$env:PATH"
    }

    if (-not (Test-ReverseIsWindows)) { return }
    ...
    if ($userEntries | Where-Object { $_.TrimEnd($sepChars) -ieq $normalized }) {
        return
    }

    $updated = (@($userEntries) + $normalized) -join ';'
    try {
        [Environment]::SetEnvironmentVariable('PATH', $updated, 'User')
    } catch {
        # Process PATH is already updated; persistence is best-effort.
    }
}
...
function Ensure-GitHubZipInstall {
    ...
    if ($existing.Available) {
        # Resolvable now, but possibly only via a PATH entry added by an earlier
        # run of this process. Persist the install dir before returning.
        if ($TargetPath) { Add-ReversePersistentUserPath -Directory $TargetPath }
        return $existing
    }
    ...
    # Refresh PATH so newly installed tools are discoverable - and persist it,
    # otherwise the tool is only callable from inside this bootstrap process.
    $binCandidates = @(
    ...
    foreach ($binDir in $binCandidates) {
        if (Test-Path -LiteralPath $binDir) {
            Add-ReversePersistentUserPath -Directory $binDir
        }
    }
...
# Write both apktool launchers into the install dir.
#
# `apktool.bat` is what Windows shells need. The extension-less `apktool` shim
# is what Git Bash / MSYS2 resolve: `command -v apktool` matches a file named
# exactly that, and does NOT match apktool.bat. The Unix-side helpers
# (testing/apk-reverse/scripts/decode.sh, testing/radare2/scripts/recon.sh)
# gate on `command -v <tool>` before use, so a .bat-only install reads as
# "not installed" and they abort with a bogus manual-install message.
#
# The shim is written LF-only: a CRLF shebang makes bash fail with
# "/usr/bin/env: 'bash\r': No such file or directory".
#
# Idempotent - safe to call on an install dir that already has both files.
function Write-ApktoolWrappers {
    param(
    ...
    $batPath = Join-Path $InstallDir $WrapperName
    @(
        '@echo off',
        'setlocal',
    ...
    $posixPath = Join-Path $InstallDir 'apktool'
    # `java` is a native binary: it does not understand an MSYS path such as
    # /c/Users/x/apktool.jar. Convert with cygpath when present (Git Bash / MSYS2
    # always ship it); if it is somehow missing, the raw path is still correct on
    # real POSIX systems, so fall back to it rather than failing.
    $posixLines = @(
        '#!/usr/bin/env bash',
        '# Generated by bootstrap-reverse.ps1 - POSIX shim for the sibling apktool.jar.',
        'set -euo pipefail',
        'HERE=${0%/*}',
        'JAR="$HERE/apktool.jar"',
        'if command -v cygpath >/dev/null 2>&1; then JAR="$(cygpath -w "$JAR")"; fi',
        'exec java -jar "$JAR" "$@"'
    )
    $posixText = ($posixLines -join "`n") + "`n"
    [System.IO.File]::WriteAllText($posixPath, $posixText, (New-Object System.Text.UTF8Encoding($false)))
}
...
function Ensure-ApktoolInstall {
    ...
    if ($existing.Available) {
        # Already on disk - but that only means "resolvable right now". Two things
        # can still be missing on an install made by an older bootstrap run:
        #   (a) a persistent PATH entry (the tool is only reachable in-process),
        #   (b) the extension-less POSIX shim, when the dir only holds apktool.bat.
        # Repair both, idempotently, instead of returning early and leaving them.
        if ($Definition.installDir -and (Test-Path -LiteralPath $Definition.installDir)) {
            Write-ApktoolWrappers -InstallDir $Definition.installDir -WrapperName $Definition.wrapperName
            Add-ReversePersistentUserPath -Directory $Definition.installDir
        }
        return $existing
    }
    ...
    Write-ApktoolWrappers -InstallDir $installDir -WrapperName $Definition.wrapperName

    Add-ReversePersistentUserPath -Directory $installDir
...
function Ensure-Capability {
    ...
    if ($existingState -and -not $definition.PSObject.Properties['mcpNames'] -and $definition.bootstrapKind -ne 'git-clone') {
        ...
        if ($toolSpec -and $toolSpec.Available) {
            # Reachable right now - but that can be true for two reasons that both
            # need repairing before we call this capability done:
            #   (a) the PATH entry lives only in this process (older bootstrap run),
            #   (b) the tool dir has a Windows-only launcher, so the Unix-side
            #       helpers that gate on `command -v <tool>` still see it as absent.
            # This branch returns before the bootstrapKind switch below, so the
            # per-kind repair has to happen here or not at all.
            if ($definition.PSObject.Properties['installDir'] -and $definition.installDir) {
                if ($definition.PSObject.Properties['wrapperName'] -and
                    $definition.wrapperName -and
                    (Test-Path -LiteralPath $definition.installDir)) {
                    Write-ApktoolWrappers -InstallDir $definition.installDir -WrapperName $definition.wrapperName
                }
                Add-ReversePersistentUserPath -Directory $definition.installDir
            }
