...
# Usage:
#   bash scripts/reverse-skill/case-init.sh --hint "apk reverse" --case-name demo
#   bash scripts/reverse-skill/case-init.sh --hint "local sample" --preset offline-sample --sample ./app.apk
#   bash scripts/reverse-skill/case-init.sh --hint "ctf web" --preset ctf-public --target-url https://chal.example
set -euo pipefail
...
- checklist:
  ...
  - [ ] roles assigned (see docs/ops/role-map.md)

## ops_refs
- docs/ops/scope-contract.md
- docs/ops/evidence-finding-path.md
- docs/ops/role-map.md
- docs/ops/timeline-workitem.md
- docs/ops/IDENTITY.md
EOF
...
## $created | lead | init
- action: case-init
- command_or_ref: scripts/reverse-skill/case-init.sh
...
5. Promote findings with Evidence chain (docs/ops/evidence-finding-path.md)
...
1. Edit \`scope.md\` — set auth.status=granted and in_scope
   - or re-run: \`bash scripts/reverse-skill/case-init.sh --hint "..." --preset offline-sample --sample ./sample.apk\`
