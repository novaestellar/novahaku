...
1. 读取共享 tool availability，先复用已安装工具。
...
7. 刷新共享 tool availability，并输出正式安装报告和流程图。
...
powershell -NoProfile -ExecutionPolicy Bypass -File "{package_root}\skills\scripts\tool discovery script"
codex mcp list
...
- 将 Dr. Memory、CDB、GFlags、UMDH、DTC、SquashFS、flashrom 和 Frida Trace 纳入 Windows tool availability catalog。
...
- 工具版本: 详见本机正式安装报告与 tool availability
