...
## 目标概述
修复本机引导过程中的三类被层层掩盖的缺陷：tool availability 把 radare2 主分析器 `r2` 误报为 no；多个测试脚本硬编码 `powershell` 子进程调用在仅装 PowerShell 7+ 的机器上失败；以及被该失败掩盖的 pin gate StrictMode 属性访问 bug。
...
1. 按 `README_AI.md` 第 0 节执行引导：`tool discovery script` 生成 tool availability reference（37 工具）。
2. 读 `tool availability reference` 发现异常：`r2`（radare2 主分析器）= no，但同目录 `rabin2/rasm2/radiff2/rahash2/rax2/r2pm` 全部 = yes。
...
6. 重跑 `tool discovery script`，`r2` 转 yes，路径 `r2.bat`，版本 `radare2 6.2.0`，来源 `FallbackPath`。
...
|------|----------|--------|-------------|----------------|--------------|
| E-r2 | info | validated | command | `pwsh -File scripts/reverse-skill/tool discovery script` 后 tool availability reference 中 `r2` 行 = yes | F-r2 |
| E-smoke | info | validated | command | `pwsh -File scripts/reverse-skill/smoke.ps1` → `OVERALL: ALL PASS` | F-host |
| E-route | info | validated | command | `pwsh -File scripts/reverse-skill/test-routing.ps1` → `166/166 ALL PASS` | F-host |
| E-p0 | info | validated | command | `pwsh -File scripts/reverse-skill/test-p0-friction.ps1` → `OVERALL: ALL PASS` | F-host |
...
| 问题 | 原因 | 解决方案 | 耗时 |
|------|------|---------|------|
| tool availability 把 `r2` 标 no，但同目录其他 r2* 工具 yes | `ToolDiscovery.ps1` 的 `r2` Fallbacks 只找 `r2.exe`，而 radare2 Windows 发行版用 `r2.bat` 包装 `radare2.exe`，无 `r2.exe` | 补 `r2.bat` 与 `radare2.exe` 路径 fallback | 短 |
...
## 进化动作
- [x] 更新了 tool availability（r2 转为 yes，路径 r2.bat）
- [x] 新增 `scripts/reverse-skill/lib/HostRuntime.ps1`
