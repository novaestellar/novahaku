...
3. If **reflected**
   1. Check **which symbols can you use** and depending on that, prepare the payload:
      1. In **raw HTML**:
         ...
         4. Is the HTML content being interpreted by any client side JS engine (_AngularJS_, _VueJS_, _Mavo_...), you could abuse a **Client Side Template Injection**.
...
A corporate SSO login page reflected the OAuth `service` parameter inside the `href` attribute of `<a id="forgot_btn" ...>`. Even though `<` and `>` were HTML-encoded, double quotes were not, so the attacker could close the attribute and reuse the same element to inject handlers such as `" onfocus="payload" x="`.<sup>[[1]](#references)</sup>
...
Javascript Hoisting - Javascript Function: A good way to find out if something given directly by the user is trying to be executed is modifying the param value (for example to....png>)
...
Javascript Hoisting - Javascript Function: However, even with that limitation it's still possible to perform some actions. This is because you can use that valid chars to access any....png>)
...
from https://twitter.com/hackerscrolls/status/1273254212546281473?s=21.jpg>)
...
// You can substitute these characters for the space between attributes:
/
/*%00/
/%00*/
%2F
...
If it is impossible to create an HTML tag with an attribute that executes JavaScript, check **Dangling Markup**: the injection may still be exploitable **without JavaScript execution**.
...
This executes attacker JS without needing to touch HTML context (pure JS-in-JS). Combine with blacklist bypasses below when filters block keywords.<sup>[[4]](#references)</sup>
...
Reference: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/eval<sup>[[5]](#references)</sup>
...
#!This is a one-line comment, but "#!" must be at the beginning of the first line
-->This is a one-line comment, but "-->" must be at the beginning of the first line
```
...
// Backtips
  // Backticks pass the string as an array of length 1
alert`1`
...
"a,".replace`a${alert}` // Initial ["a"] is passed as "a,", so the initial string is "a,"
...
"a".replace.call`1337${/..../}${alert}` //alert with 1337 instead
  ...
  // Use Reflect.apply to call any function with arbitrary arguments
...
You can find a great abuse of this technique in [**this blog post**](https://nokline.github.io/bugbounty/2024/06/07/Zoom-ATO.html).<sup>[[9]](#references)</sup>
...
Because of **Ruby on Rails mass assignment**, quotes are inserted into the HTML, bypassing the quote restriction and allowing additional fields such as `onfocus` inside the tag.\
Form example ([from this report](https://hackerone.com/reports/709336)), if you send the payload:<sup>[[10]](#references)</sup>
...
This behaviour was used in [**this writeup**](https://github.com/zwade/yaca/tree/master/solution) to remap a library to eval to abuse it can trigger XSS.<sup>[[13]](#references)</sup>
...
If the page returns a `text/xml` content type, a namespace declaration may enable arbitrary JavaScript execution:
...
For example in [**this writeup**](https://gitea.nitowa.xyz/nitowa/PlaidCTF-YACA), this was used to **scape a JSON string** inside a script and execute arbitrary code.<sup>[[14]](#references)</sup>
...
[According to this](https://stackoverflow.com/questions/28955047/why-does-a-module-level-return-statement-work-in-node-js/28955050#28955050) modules are wrapped by Node.js within a function, like this:<sup>[[16]](#references)</sup>
...
- More sophisticated JSFuck: [https://medium.com/@Master_SEC/bypass-uppercase-filters-like-a-pro-xss-advanced-methods-daf7a82673ce](https://medium.com/@Master_SEC/bypass-uppercase-filters-like-a-pro-xss-advanced-methods-daf7a82673ce)
...
If a page **stores `event.origin` from a `postMessage` and later concatenates it into a script URL**, the sender controls the **origin** of the loaded JS:<sup>[[6]](#references)</sup>
...
When a backend **builds a shared SDK by concatenating JS strings with user-controlled values**, any quote/structure breaker can inject script that is served to every consumer:<sup>[[6]](#references)</sup>
...
If uploaded files are parsed and their metadata is printed into HTML reports with escaping disabled (`|safe`, custom renderers), that metadata is a **stored XSS sink**. Example flow:<sup>[[7]](#references)</sup>
...
Example [**writeup XSS in Amp4Email in Gmail**](https://adico.me/post/xss-in-gmail-s-amp4email).<sup>[[18]](#references)</sup>
...
- Because the unsubscribe helper often reuses the same HTTP stack as the application, you inherit its proxy settings, HTTP verbs, and header rewrites, enabling further traversal tricks described in the SSRF methodology.
...
- [9] [nokline.github.io - 07 - Zoom ATO](https://nokline.github.io/bugbounty/2024/06/07/Zoom-ATO.html)
- [10] [hackerone.com - from this report](https://hackerone.com/reports/709336)
- [11] [gremwell.com - Firefox Xss 302](https://www.gremwell.com/firefox-xss-302)
- [12] [hahwul.com - Forcing Http Redirect Xss](https://www.hahwul.com/2020/10/03/forcing-http-redirect-xss)
- [13] [zwade/yaca](https://github.com/zwade/yaca/tree/master/solution)
- [14] [gitea.nitowa.xyz - Nitowa - PlaidCTF YACA](https://gitea.nitowa.xyz/nitowa/PlaidCTF-YACA)
- [15] [Huli - What I Learned From Dicectf 2022 - Index: Miscx2fundefined55 Solves](https://blog.huli.tw/2022/02/08/en/what-i-learned-from-dicectf-2022/index.html#miscx2fundefined55-solves)
- [16] [stackoverflow.com - According to this](https://stackoverflow.com/questions/28955047/why-does-a-module-level-return-statement-work-in-node-js/28955050#28955050)
- [17] [blog.arkark.dev - 18 - Seccon En: Web Piyosay](https://blog.arkark.dev/2022/11/18/seccon-en/#web-piyosay)
- [18] [adico.me - Xss In Gmail S Amp4email](https://adico.me/post/xss-in-gmail-s-amp4email)
