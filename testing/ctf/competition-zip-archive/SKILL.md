...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/reverse-skill/refresh-tool-index.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/reverse-skill/bootstrap-reverse.ps1 -Capability bkcrack
```
...
The Windows capability is pinned to the `v1.8.1` release and verifies the GitHub asset digest. If the tool is installed manually, refresh `tool availability reference` afterward.
