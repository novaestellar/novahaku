...
| "我没找到 tool-index，我就直接猜路径" | **缺文件比猜错路径安全 100 倍。** tool-index 缺失时先运行 tool check 生成。 |
...
3. **NEXT**: 读取 `novahaku/SKILL.md` → `novahaku routing` → 确定进入哪个子 skill
4. **NEXT**: 读取 `tool availability` 确认工具状态
...
2. 检查本机是否有所需工具（读 `../tool availability`）
...
□ 我实际执行了行为链中的每一步（没有跳过）
□ 我没有猜测任何工具路径（都来自 tool availability）
